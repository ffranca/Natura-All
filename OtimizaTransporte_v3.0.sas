...
/*CADASTRO_FILIAIS*/
PROC SQL;
   CREATE TABLE WORK.CADASTRO_FILIAIS AS 
   SELECT t1.COD_CD, 
          t1.CD, 
          t1.TRANSPORTADORA, 
          t1.FILIAL, 
          CATX(" ", t1.UF,t1.CIDADE) AS 'UF CIDADE FILIAL'n, 
          t1.'Vel Média -TP ZN'n, 
          t1.'TEMPO MOVIM CARGA'n, 
          t1.'TIPO TURNO MOV'n, 
          t1.'TIPO TURNO EXP'n, 
          t1.'TEMPO ENTREGA'n, 
          /* MIN_of_DATA_INI */
...
/*ITINERARIO*/
PROC SQL;
   CREATE TABLE WORK.ITINERARIO AS 
   SELECT t1.ZONEAMENTO, 
          t1.COD_CD AS CD, 
          t1.ROTA, 
          t1.DURACAO_TOTAL, 
          t1.FILIAL, 
          t1.DURACAO_CD_FILIAL, 
          /* DATA_INI */
            (MIN(t1.DATA_INI)) FORMAT=DATE9. AS DATA_INI, 
          /* DATA_FIN */
            (MAX(t1.DATA_FIN)) FORMAT=DATE9. AS DATA_FIN
      FROM SIMULA.ESTRUTURA_LOGISTICA t1
      GROUP BY t1.ZONEAMENTO,
               t1.CD,
               t1.ROTA,
               t1.DURACAO_TOTAL,
               t1.FILIAL,
...
/* CADASTRO_FILIAIS*/
	...
	read data work.cadastro_filiais into FilialSet=[FILIAL]
...
/* Carrega Prazo*/
	...
	read data WORK.ROTA_ZONEAMENTO into RotaZone=[ROTA ZONEAMENTO DATA_INI DATA_FIN]
		prazo_contratado;
...
PROC SQL;
   ...
   SELECT t1.COD_CD, 
	  ...
	  /* FROM SIMULA.SAIDA_CAPTACAO2 t1 */
...
PROC SQL;
   CREATE TABLE WORK.SAIDA_NS_PONDERADO AS SELECT DISTINCT
      ...
      FROM SIMULA.SAIDA_NS_DETALHADO3 t1
...
PROC SQL;
   ...
   SELECT DISTINCT t1.*, 
          t2.*
      FROM WORK.DATA_HORA_EXP_ZN t1 left JOIN WORK.CALENDARIZACAO t2 ON (t1.ROTA = t2.ROTA)
...
PROC SQL;
   ...
   SELECT distinct t2.ROTA, 
      ...
      FROM DATA_HORA_EXP_ZN t1, WORK.ITINERARIO t2
...
PROC SQL;								
   CREATE TABLE WORK.SAIDA_NSU_PONDERADO AS SELECT DISTINCT								
		t1.COD_CD, 						
          ...
          /* VOLUME */								
            (SUM(t1.VOLUME)) FORMAT=COMMA12.2 AS VOLUME								
      FROM SIMULA.SAIDA_NSU_DETALHADO3 t1
...
/*            ((t1.ns_cc) + (t1.NS + t1.tempo_separacao + t1.tempo_expedicao)+ NS_TRANSP_CAP_UTEIS) AS NS_TOTAL_CAP_UTEIS,*/
      ...
      FROM WORK.NS_TRANSP_002 t1, SIMULA.SAIDA_DEMANDA_DETALHADA3 t2 /* SIMULA.SAIDA_DEMANDA_DETALHADA2 t2	*/							
...
PROC SQL noprint;
   ...
   SELECT DISTINCT t1.FILIAL
      FROM WORK.CADASTRO_FILIAIS t1 
		INNER JOIN WORK.DATA_ITINERARIO t2 ON t1.FILIAL=t2.FILIAL;
